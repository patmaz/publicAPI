...
const getWordsFilter = async () => {
  try {
    const result = await promise({
      ...
      }.cloudfunctions.net/filterWords`,
      method: 'POST',
      json: true,
    });
    ...
    promise(options)
      .then(async $ => {
        ...
        links.each((index, link) => {
          const nakedLink = $(link)
            .attr('href')
            .replace('/url?q=', '')
            .split('&')[0];
          if (nakedLink.indexOf('http') === 0) {
            promises.push(promise(nakedLink));
          }
        });
  ...
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < promises.length; i = i + batchSize) {
      ...
      console.log(
        ...
        promises.slice(start, end).length,
      );
      q.all(promises.slice(start, end)).then(
        data => {
          ...
          if (
            resolvedBatchesCounter === Math.ceil(promises.length / batchSize)
          ) {
            resolve(result);
          }
        },
